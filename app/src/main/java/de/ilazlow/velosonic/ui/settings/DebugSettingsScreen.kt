...
fun DebugSettingsScreen(onBack: () -> Unit, onViewFullLog: () -> Unit, viewModel: DebugSettingsViewModel = hiltViewModel()) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        SettingsTopBar("Debug", onBack)
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            item {
                SettingsSwitchRow(
                    title = "Enable Logging",
                    subtitle = "Keeps a rolling in-app log for troubleshooting",
            ...
            if (settings.loggingEnabled) {
                ...
                item {
                    Row(
                        ...
                        Text(text = "View Full Log", style = MaterialTheme.typography.bodyLarge, modifier = Modifier.weight(1f))
                ...
                item {
                    Row(
                        ...
                        TextButton(onClick = {
                            ...
                            Text(if (justCopied) "Copied" else "Copy Log")
                        }
                    }
                }
                item { HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp)) }
                item { SettingsSectionHeader("Share") }
                item {
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                        ...
                        Text(text = "Share Log", style = MaterialTheme.typography.bodyLarge)
                    }
                }
                item {
                    Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 20.dp, vertical = 8.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                        Text("Log File", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                ...
                if (otherFiles.isNotEmpty()) {
                    item { HorizontalDivider(modifier = Modifier.padding(vertical = 8.dp)) }
                    item { SettingsSectionHeader("All Logs") }
    ...
    if (confirmClear) {
        AlertDialog(
            ...
            text = { Text("Deletes every stored log file, including the current session's.") },
            confirmButton = {
